...
###Config Git
	...
	$ git config --global user.name "jmt.im"  
...
###Pull in source code
	$ cd ~/www/
	$ git clone git@github.com:gaolinjie/jmt.im.git
	$ cd jmt.im
...
###Create symbolic links to conf files
	$ cd /etc/nginx 
	$ rm nginx.conf
	$ ln -s ~/www/jmt.im/conf/nginx.conf nginx.conf 
	$ cd
	$ ln -s ~/www/jmt.im/conf/supervisord.conf supervisord.conf  
